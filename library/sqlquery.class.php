...
class SQLQuery {
    ...
    function whereArray($arrayData = array()) {
	...
	function search() {
		...
		if ($this->_hO == 1 && isset($this->hasOne)) {
			foreach ($this->hasOne as $alias => $model) {
				...
				$from .= 'ON `'.$this->_model.'`.`id'.$singularAlias.'` = `'.$alias.'`.`id`  ';
			}
		}
			...
			while ($row = mysql_fetch_row($this->_result)) {
				...
				if ($this->_hM == 1 && isset($this->hasMany)) {
					...
					foreach ($this->hasMany as $aliasChild => $modelChild) {
                        ...
                        $conditionsChild .= '`'.$aliasChild.'`.`id` = \''.$tempResults[$this->_model]['id'].'\'';
	
						$queryChild =  'SELECT * FROM '.$fromChild.' WHERE '.$conditionsChild;	
						#echo '<!--'.$queryChild.'-->';
                        //var_dump($queryChild.'<br/>');
				...
				if ($this->_hMABTM == 1 && isset($this->hasManyAndBelongsToMany)) {
					foreach ($this->hasManyAndBelongsToMany as $aliasChild => $tableChild) {
                        ...
                        $conditionsChild .= '`'.$joinTable.'`.`id` = `'.$aliasChild.'`.`id` AND ';
						$conditionsChild .= '`'.$joinTable.'`.`id` = \''.$tempResults[$this->_model]['id'].'\'';
	...
	function save($vals = array()) {
		$query = '';        
		if (isset($this->id) && ($this->_mode == 'update')) {
			$updates = '';
